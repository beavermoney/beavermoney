...
func (_q *SnapshotQuery) QueryHousehold() *HouseholdQuery {
...
func (_q *SnapshotQuery) QuerySnapshotEntries() *SnapshotEntryQuery {
...
func (_q *SnapshotQuery) WithHousehold(opts ...func(*HouseholdQuery)) *SnapshotQuery {
	query := (&HouseholdClient{config: _q.config}).Query()
...
func (_q *SnapshotQuery) WithSnapshotEntries(opts ...func(*SnapshotEntryQuery)) *SnapshotQuery {
	query := (&SnapshotEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(snapshot.FieldHouseholdID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Snapshot.Query().
//		Select(snapshot.FieldHouseholdID).
//		Scan(ctx, &v)
...
func (_q *SnapshotQuery) prepareQuery(ctx context.Context) error {
	...
	if err := snapshot.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *SnapshotQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Snapshot, error) {
	...
	if query := _q.withHousehold; query != nil {
		if err := _q.loadHousehold(ctx, query, nodes, nil,
			func(n *Snapshot, e *Household) { n.Edges.Household = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSnapshotEntries; query != nil {
		if err := _q.loadSnapshotEntries(ctx, query, nodes,
			func(n *Snapshot) { n.Edges.SnapshotEntries = []*SnapshotEntry{} },
			func(n *Snapshot, e *SnapshotEntry) { n.Edges.SnapshotEntries = append(n.Edges.SnapshotEntries, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SnapshotQuery) loadHousehold(ctx context.Context, query *HouseholdQuery, nodes []*Snapshot, init func(*Snapshot), assign func(*Snapshot, *Household)) error {
...
func (_q *SnapshotQuery) loadSnapshotEntries(ctx context.Context, query *SnapshotEntryQuery, nodes []*Snapshot, init func(*Snapshot), assign func(*Snapshot, *SnapshotEntry)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(snapshotentry.FieldSnapshotID)
	}
...
func (_q *SnapshotQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHousehold != nil {
			_spec.Node.AddColumnOnce(snapshot.FieldHouseholdID)
		}
	}
...
func (_q *SnapshotQuery) WithNamedSnapshotEntries(name string, opts ...func(*SnapshotEntryQuery)) *SnapshotQuery {
	query := (&SnapshotEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
