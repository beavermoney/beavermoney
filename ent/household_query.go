...
func (_q *HouseholdQuery) QueryInvestmentLots() *InvestmentLotQuery {
...
func (_q *HouseholdQuery) QuerySnapshotRates() *SnapshotRateQuery {
...
func (_q *HouseholdQuery) WithInvestmentLots(opts ...func(*InvestmentLotQuery)) *HouseholdQuery {
	query := (&InvestmentLotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *HouseholdQuery) WithSnapshotRates(opts ...func(*SnapshotRateQuery)) *HouseholdQuery {
	query := (&SnapshotRateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *HouseholdQuery) prepareQuery(ctx context.Context) error {
	...
	if household.Policy == nil {
		return errors.New("ent: uninitialized household.Policy (forgotten import ent/runtime?)")
	}
	if err := household.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *HouseholdQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Household, error) {
	...
	if query := _q.withInvestmentLots; query != nil {
		if err := _q.loadInvestmentLots(ctx, query, nodes,
			func(n *Household) { n.Edges.InvestmentLots = []*InvestmentLot{} },
			func(n *Household, e *InvestmentLot) { n.Edges.InvestmentLots = append(n.Edges.InvestmentLots, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSnapshotRates; query != nil {
		if err := _q.loadSnapshotRates(ctx, query, nodes,
			func(n *Household) { n.Edges.SnapshotRates = []*SnapshotRate{} },
			func(n *Household, e *SnapshotRate) { n.Edges.SnapshotRates = append(n.Edges.SnapshotRates, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedInvestmentLots {
		if err := _q.loadInvestmentLots(ctx, query, nodes,
			func(n *Household) { n.appendNamedInvestmentLots(name) },
			func(n *Household, e *InvestmentLot) { n.appendNamedInvestmentLots(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedSnapshotRates {
		if err := _q.loadSnapshotRates(ctx, query, nodes,
			func(n *Household) { n.appendNamedSnapshotRates(name) },
			func(n *Household, e *SnapshotRate) { n.appendNamedSnapshotRates(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *HouseholdQuery) loadInvestmentLots(ctx context.Context, query *InvestmentLotQuery, nodes []*Household, init func(*Household), assign func(*Household, *InvestmentLot)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Household)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.InvestmentLot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(household.InvestmentLotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *HouseholdQuery) loadSnapshotRates(ctx context.Context, query *SnapshotRateQuery, nodes []*Household, init func(*Household), assign func(*Household, *SnapshotRate)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Household)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(snapshotrate.FieldHouseholdID)
	}
	query.Where(predicate.SnapshotRate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(household.SnapshotRatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *HouseholdQuery) WithNamedInvestmentLots(name string, opts ...func(*InvestmentLotQuery)) *HouseholdQuery {
	query := (&InvestmentLotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *HouseholdQuery) WithNamedSnapshotRates(name string, opts ...func(*SnapshotRateQuery)) *HouseholdQuery {
	query := (&SnapshotRateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
