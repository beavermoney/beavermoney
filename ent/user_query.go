...
func (_q *UserQuery) QueryInvestments() *InvestmentQuery {
...
func (_q *UserQuery) QueryRecurringSubscriptions() *RecurringSubscriptionQuery {
...
func (_q *UserQuery) WithInvestments(opts ...func(*InvestmentQuery)) *UserQuery {
	query := (&InvestmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithRecurringSubscriptions(opts ...func(*RecurringSubscriptionQuery)) *UserQuery {
	query := (&RecurringSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withInvestments; query != nil {
		if err := _q.loadInvestments(ctx, query, nodes,
			func(n *User) { n.Edges.Investments = []*Investment{} },
			func(n *User, e *Investment) { n.Edges.Investments = append(n.Edges.Investments, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withRecurringSubscriptions; query != nil {
		if err := _q.loadRecurringSubscriptions(ctx, query, nodes,
			func(n *User) { n.Edges.RecurringSubscriptions = []*RecurringSubscription{} },
			func(n *User, e *RecurringSubscription) {
	...
	for name, query := range _q.withNamedInvestments {
		if err := _q.loadInvestments(ctx, query, nodes,
			func(n *User) { n.appendNamedInvestments(name) },
			func(n *User, e *Investment) { n.appendNamedInvestments(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadInvestments(ctx context.Context, query *InvestmentQuery, nodes []*User, init func(*User), assign func(*User, *Investment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(investment.FieldUserID)
	}
	query.Where(predicate.Investment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.InvestmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadUserKeys(ctx context.Context, query *UserKeyQuery, nodes []*User, init func(*User), assign func(*User, *UserKey)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userkey.FieldUserID)
	}
...
func (_q *UserQuery) loadRecurringSubscriptions(ctx context.Context, query *RecurringSubscriptionQuery, nodes []*User, init func(*User), assign func(*User, *RecurringSubscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(recurringsubscription.FieldUserID)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithNamedInvestments(name string, opts ...func(*InvestmentQuery)) *UserQuery {
	query := (&InvestmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithNamedRecurringSubscriptions(name string, opts ...func(*RecurringSubscriptionQuery)) *UserQuery {
	query := (&RecurringSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
